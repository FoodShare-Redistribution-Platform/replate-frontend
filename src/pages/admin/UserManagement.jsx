...
const Toast = ({ message, type, onClose }) => (
    <div className={`fixed top-6 right-6 z-[1100] flex items-center gap-3 px-5 py-3.5 rounded-xl border shadow-2xl backdrop-blur-xl ${type === 'success'
...
const StatusBadge = ({ status }) => {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium capitalize ${active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
            }`}>
                ...
                {getRange().map((p, i) =>
                    p === '...' ? (
                        ...
                        <button
                            ...
                            className={`min-w-[32px] h-8 rounded-lg text-xs font-medium transition-all ${p === page
                ...
                {Object.entries(ROLE_CONFIG).map(([key, val]) => {
                    ...
                    return (
                        <button
                            ...
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-all ${active
